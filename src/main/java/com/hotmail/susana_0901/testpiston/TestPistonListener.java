...
public class TestPistonListener implements Listener {

	//-------------------------------------------------
	/*@EventHandler
	public void onPhisicEvent(BlockPhysicsEvent event) {
		Logger log;
		// TODO Auto-generated method stub
		log = Logger.getLogger("Minecraft");
		
		Location loc = event.getBlock().getLocation();
		log.info("evento physic se activo por bloque "+event.getChangedType()+loc);
	}*/
	//-------------------------------------------------

	public TestPiston vspp;
	public boolean isEnabled;

	public TestPistonListener(TestPiston pg)
	{
		vspp = pg;
		isEnabled = false;
	}
	...
	{
        	 vspp.getLogger().info("se ha extendido un piston");
		if(!isEnabled)
		{
			return;
		}
		...
		for(int count = 0;count<mv.size();count++)
		{
			...
			tb = mv.get(count);
			 vspp.getLogger().info("\n Lista mv "+count+" tiene "+tb.getType().toString()+", getBlockPower="+tb.getBlockPower()+"\n");
	...
	{
        	vspp.getLogger().info("se ha Retraido un piston");
		if(!isEnabled)
		{
			return;
		}
